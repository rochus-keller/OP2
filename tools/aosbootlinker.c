...
*
* Boot linker for the AOS (Bluebottle) operating system
*
* Standalone C99 port derived from multibootlinker.c, adapted for AOS.
* Emulates the AOS heap layout (NewSys, NewType, AllocateModule) and
* module descriptor format (AosModules.Module, 172 bytes).
* Uses VarByName to dynamically patch root/initBlock in AosModules.
*
* Notes:
* - Targets 32-bit little-endian OM object files (OFVersion 0x0AF).
* - SysFixes point to AosHeap / AosActive kernel procedures.
* - Command names starting with '$' are treated as parameterized commands.
...
*
* compile e.g. with:
*   cc -std=c99 -O2 -Wall -Wextra -o aosbootlinker aosbootlinker.c
*
...
enum {
    /* SysFix constants – AOS kernel targets */
    newSF = 0,          // "new", "AosHeap.NewRec"
    sysnewSF = 1,       // "sysnew", "AosHeap.NewSys"
    newarrSF = 2,       // "newarr", "AosHeap.NewArr"
    StartSF = 3,        // "start", "AosActive.CreateProcess"
    PassivateSF = 4,    // "passivate", "AosActive.Await"
    ActivateSF = 5,     // "activate", ""
    LockSF = 6,         // "lock", "AosActive.Lock"
    UnlockSF = 7,       // "unlock", "AosActive.Unlock"
    divmodSF = 8,       // "divmod", "DivSupport.DivMod32"
    /* 9 reserved */
    copyarraySF = 10,
    CurProcSF = 11,     // "", ""
    commandSF = 12,     // "command", ""
    /* 13..16 removed (listSF, modDescSF, expDescSF not used in AOS) */
    objectSF = 16,      // "", ""
...
enum {
    ...
    /* AOS heap constants (cf. AosHeap, AosLinker0) */
    AosBlockSize = 32,
    AosProtOfs = 48,           /* AosBlockSize + 16 */
    AosMinPtrOfs = (int32_t)0xC0000000, /* -0x40000000 */
    AosModuleDescSize = 172,   /* sizeof(AosModules.Module) */
    AosCommandRecSize = 40,    /* Name(32) + proc(4) + parproc(4) */
    AosArrHdrSize = 16,        /* array header: 4 fields of 4 bytes */
    AosArrLenOfs = 12,         /* offset of dimension 0 in array header */
...
typedef struct {
    Name name;
    int32_t adr;
    bool has_param; /* AOS: true if command name started with '$' */
} CommandDesc;
...
/* AOS heap size helpers (cf. AosLinker0/AosLinker AllocateModule) */

static int32_t aos_arr_size(int32_t elements, int32_t elemSize) {
    return ((elements * elemSize + AosArrHdrSize + 4 + 15) / 16) * 16;
}

static int32_t aos_newsys_block_size(int32_t payload) {
    return (payload + 4 + 24 + AosBlockSize - 1) & ~(AosBlockSize - 1);
}

static int32_t aos_newmodule_block_size(void) {
    return (AosModuleDescSize + 4 + AosProtOfs + AosBlockSize - 1) & ~(AosBlockSize - 1);
}
...
/* AOS does not use DumpModuleDesc; module descriptor is 172 bytes in-image */
...
static void initialise(void) {
    ...
    /* AOS kernel targets – always resolved automatically */
    init_sysfix(newSF, "new", "AosHeap", "NewRec", true);
    init_sysfix(sysnewSF, "sysnew", "AosHeap", "NewSys", true);
    init_sysfix(newarrSF, "newarr", "AosHeap", "NewArr", true);

    init_sysfix(StartSF, "start", "AosActive", "CreateProcess", true);
    init_sysfix(PassivateSF, "passivate", "AosActive", "Await", true);
    init_sysfix(ActivateSF, "activate", "", "", 0);
    init_sysfix(LockSF, "lock", "AosActive", "Lock", true);
    init_sysfix(UnlockSF, "unlock", "AosActive", "Unlock", true);
    init_sysfix(divmodSF, "divmod", "DivSupport", "DivMod32", true);

    /* Future soft-float64 sysfixes (unused by ARM32 VFPv3 backend, for RV32IMAFC) */
    init_sysfix(f64addSF, "f64add", "Float64", "Add", true);
    init_sysfix(f64subSF, "f64sub", "Float64", "Sub", true);
    init_sysfix(f64mulSF, "f64mul", "Float64", "Mul", true);
    init_sysfix(f64divSF, "f64div", "Float64", "Div", true);
    init_sysfix(f64cmpSF, "f64cmp", "Float64", "Cmp", true);
    init_sysfix(f64cvtifSF, "f64cvtif", "Float64", "CvtIF", true);
    init_sysfix(f64cvtfiSF, "f64cvtfi", "Float64", "CvtFI", true);
    init_sysfix(f64negSF, "f64neg", "Float64", "Neg", true);
    init_sysfix(f64absSF, "f64abs", "Float64", "Abs", true);
...
static void read_cmd(FILE *f, Module *m) {
    ...
    for (int32_t i = 0; i < m->nofCmds; i++) {
        ...
        /* AOS: strip leading '$' and mark as parameterized command */
        if (m->cmds[i].name[0] == '$') {
            m->cmds[i].has_param = true;
            memmove(m->cmds[i].name, m->cmds[i].name + 1, strlen(m->cmds[i].name));
        } else {
            m->cmds[i].has_param = false;
        }
...
static void write_zeros(FILE *out, int32_t n);
...
/* VarByName - Find a global variable offset in the reference block.
 * Translated from AosLinker0.Mod VarByName procedure.
 * Returns the variable offset (relative to sb), or 0 if not found. */
static int32_t aos_var_by_name(const uint8_t *refs, int32_t refSize, const char *name) {
    int32_t i = 0, m = refSize;
    if (i >= m) return 0;

    /* Skip the refs header: expect 0xF8, 0x00, "$$", 0x00 */
    if (refs[i] == 0xF8) {
        i++;
        if (i < m && refs[i] == 0x00) i++;
        /* skip "$$\0" */
        while (i < m && refs[i] != 0) i++;
        if (i < m) i++; /* skip terminating 0 */
    }
    if (i >= m) return 0;

    uint8_t mode = refs[i]; i++;
    while (i < m && (mode == 0x01 || mode == 0x03)) { /* var modes */
        uint8_t type = refs[i]; i++;
        if (type >= 0x81 || type == 0x16 || type == 0x1D) {
            /* skip dim/tdadr (compressed number) */
            int32_t dummy = 0;
            get_num_from_refs(refs, &i, &dummy);
        }
        int32_t adr = 0;
        get_num_from_refs(refs, &i, &adr);
        ...
        if (i < m) { mode = refs[i]; i++; }
        else break;
    }
    return 0;
}

/* Find a module by name in the objectList */
...
/* Find the address of a named command in a module's command table */
static int32_t find_command_adr(Module *m, const char *cmdName) {
    for (int32_t i = 0; i < m->nofCmds; i++) {
        if (strcmp(m->cmds[i].name, cmdName) == 0)
            return m->cmds[i].adr;
    }
    return 0;
}
...
static void dump_init_calls(FILE *out, int32_t *entry, int32_t stack_size, int32_t image_base, bool multiboot) {
    ...
    /* +1 call for AosActive.Terminate at the end */
    int32_t initCodeSize = (nofEntryPoints + 1) * arch->call_instr_size + arch->halt_seq_size + preamble_size;

    /* AOS: init code is wrapped in a NewSys block instead of ptr_header */
    int32_t blkSize = aos_newsys_block_size(initCodeSize);
    int32_t blkBase = *entry;
    int32_t sysTag = blkBase + 4;

    /* Write NewSys header */
    write_i32_le(out, sysTag);
    write_i32_le(out, blkSize - 4);
    write_i32_le(out, AosMinPtrOfs - 4);
    write_i32_le(out, 0);
    write_i32_le(out, 0);
    write_i32_le(out, 0);
    write_i32_le(out, sysTag);

    int32_t initCodeStart = blkBase + 28;
    log_puts("Init code at "); log_hex(initCodeStart); log_ln();
    int32_t pc = initCodeStart;
    ...
    if (stack_size > 0) {
        int32_t initial_sp = image_base + imageSize + blkSize;
    ...
    /* AOS: Call AosActive.Terminate */
    {
        Module *activeM = find_module_by_name("AosActive");
        if (activeM) {
            int32_t termAdr = find_command_adr(activeM, "Terminate");
            if (termAdr != 0) {
                log_puts("Terminate at "); log_hex(termAdr); log_ln();
                arch->WriteCallInstruction(out, termAdr, pc);
                pc += arch->call_instr_size;
            } else {
                log_puts("WARNING: AosActive.Terminate command not found\n");
                /* Still emit a NOP-sized call to keep sizes consistent */
                arch->WriteCallInstruction(out, pc + arch->call_instr_size, pc);
                pc += arch->call_instr_size;
            }
        } else {
            log_puts("WARNING: AosActive module not found\n");
            arch->WriteCallInstruction(out, pc + arch->call_instr_size, pc);
            pc += arch->call_instr_size;
        }
    }
    ...
    /* Pad to block size */
    int32_t written = pc - initCodeStart + arch->halt_seq_size;
    int32_t blkPad = blkSize - 28 - written;
    if (blkPad > 0) write_zeros(out, blkPad);

    /* AOS: Patch root and initBlock pointers using VarByName */
    {
        Module *modsM = find_module_by_name("AosModules");
        if (modsM && modsM->refs && modsM->refSize > 0) {
            /* Find 'root' variable offset */
            int32_t rootOfs = aos_var_by_name(modsM->refs, modsM->refSize, "root");
            if (rootOfs != 0) {
                /* Patch root to point to first module's descriptor */
                put_dword(modsM->data + modsM->dataSize, rootOfs, objectList ? objectList->modDescAdr : 0);
                log_puts("Patched AosModules.root at sb+"); log_int(rootOfs); log_ln();
            } else {
                log_puts("WARNING: AosModules.root not found in refs\n");
            }

            /* Find 'initBlock' variable offset */
            int32_t initOfs = aos_var_by_name(modsM->refs, modsM->refSize, "initBlock");
            if (initOfs != 0) {
                /* initBlock points to the init code (after NewSys header) */
                put_dword(modsM->data + modsM->dataSize, initOfs, initCodeStart);
                log_puts("Patched AosModules.initBlock at sb+"); log_int(initOfs); log_ln();
            } else {
                log_puts("WARNING: AosModules.initBlock not found in refs\n");
            }
        } else {
            log_puts("WARNING: AosModules module not found or has no refs\n");
        }
    }

    *entry = blkBase + blkSize;
}
...
/* Write one AOS sub-array inside a NewSys block.
 * tag = indirect tag value (points to NewSys tag).
 * elements / elemSize = array dimensions.
 * Returns the padded size consumed (== aos_arr_size). */
static int32_t write_aos_arr(FILE *out, int32_t tag, int32_t elements, int32_t elemSize) {
    int32_t totalSize = aos_arr_size(elements, elemSize);
    int32_t dataBytes = elements * elemSize;
    int32_t padBytes = totalSize - AosArrHdrSize - 4 - dataBytes;
    write_i32_le(out, tag);          /* indirect tag of *next* sub-obj */
    write_i32_le(out, 0);            /* array hdr field a */
    write_i32_le(out, 0);            /* array hdr field b */
    write_i32_le(out, 0);            /* array hdr field c */
    write_i32_le(out, elements);     /* dimension 0 */
    /* element data will be written by caller; emit placeholder zeros */
    (void)dataBytes; (void)padBytes;
    return totalSize;
}
...
static void write_zeros(FILE *out, int32_t n) {
    while (n > 0) {
        int32_t chunk = n > PaddingSize ? PaddingSize : n;
        write_bytes(out, padding, (size_t)chunk);
        n -= chunk;
    }
}
...
static void dump_modules(FILE *out) {
    ...
    while (m) {
        ...
        dump_addr(m->base, "Base");
        assert(tell_abs(out) + imageBase == m->base);

        int32_t modBlkSize = aos_newmodule_block_size();

        /* ---- 1) Module descriptor block (protected object) ---- */
        /* Layout: [tag(4)] [protHdr(AosProtOfs bytes)] [172-byte descriptor] [pad] */
        {
            uint8_t modBlk[256]; /* modBlkSize <= 256 */
            assert(modBlkSize <= (int32_t)sizeof(modBlk));
            memset(modBlk, 0, (size_t)modBlkSize);

            /* tag at offset 0: will point to the Module type descriptor.
             * For boot image we leave it 0 (patched by kernel at load time). */

            /* The 172-byte module descriptor starts at offset 4 + AosProtOfs */
            int32_t dOfs = 4 + AosProtOfs; /* = 52 */

            /* AOS Module descriptor fields (172 bytes):
             *   0: next(4) 4: name(32) 36: init(4) 40: published(4)
             *  44: refcnt(4) 48: sb(4)
             *  52: entry(4) 56: command(4) 60: ptrAdr(4) 64: type(4)
             *  68: module(4) 72: data(4) 76: code(4) 80: refs(4)
             *  84: entries(4) 88: commands(4) 92: pointers(4) 96: types(4)
             * 100: modules(4) 104: dataSize(4) 108: constSize(4) 112: codeSize(4)
             * 116: refSize(4) 120: CRC(4)
             * 124: export.fp(4) 128: export.adr(4) 132: export.nofExp(4) 136: export.dsc(4)
             * 140: import(4) 144: struct(4) 148: reimp(4) 152: term(4)
             * 156: publics(4) 160: privates(4) 164: nofimp(4) 168: nofstrc(4)
             */

            /* Compute sub-array addresses (same logic as load_module_body) */
            int32_t dataSizePad = m->dataSize + ((-m->dataSize) & 7);
            int32_t dataBlkBase = m->base + modBlkSize;
            int32_t arrOfs = dataBlkBase + 28; /* after NewSys header */

            int32_t entryAdr = arrOfs;
            arrOfs += aos_arr_size(m->nofEntries, 4);
            int32_t cmdAdr = arrOfs;
            arrOfs += aos_arr_size(m->nofCmds, AosCommandRecSize);
            int32_t ptrAdr = arrOfs;
            arrOfs += aos_arr_size(m->nofPtrs, 4);
            int32_t typeAdr = arrOfs;
            arrOfs += aos_arr_size(m->nofTds, 4);
            int32_t modAdr = arrOfs;
            arrOfs += aos_arr_size(m->nofImps, 4);
            int32_t dataAdr = arrOfs;
            arrOfs += aos_arr_size(dataSizePad + m->conSize, 1);
            int32_t codeAdr = arrOfs;
            arrOfs += aos_arr_size(m->codeSize, 1);
            int32_t refsAdr = arrOfs;

#define PUT32(buf, off, val) do { \
    uint32_t v_ = (uint32_t)(val); \
    (buf)[(off)+0] = (uint8_t)(v_); \
    (buf)[(off)+1] = (uint8_t)(v_ >> 8); \
    (buf)[(off)+2] = (uint8_t)(v_ >> 16); \
    (buf)[(off)+3] = (uint8_t)(v_ >> 24); \
} while(0)

            /* next (link) */
            PUT32(modBlk, dOfs + 0, prevmod);
            /* name */
            memcpy(modBlk + dOfs + 4, m->name, 32);
            /* init */
            PUT32(modBlk, dOfs + 36, m->init ? 1 : 0);
            /* published */
            PUT32(modBlk, dOfs + 40, 1);
            /* refcnt */
            PUT32(modBlk, dOfs + 44, m->refcnt > 0 ? (uint32_t)m->refcnt : 1);
            /* sb */
            PUT32(modBlk, dOfs + 48, (uint32_t)m->sb);
            /* array pointers (point to array header, i.e. first byte after indirect tag) */
            PUT32(modBlk, dOfs + 52, (uint32_t)entryAdr);
            PUT32(modBlk, dOfs + 56, (uint32_t)cmdAdr);
            PUT32(modBlk, dOfs + 60, (uint32_t)ptrAdr);
            PUT32(modBlk, dOfs + 64, (uint32_t)typeAdr);
            PUT32(modBlk, dOfs + 68, (uint32_t)modAdr);
            PUT32(modBlk, dOfs + 72, (uint32_t)dataAdr);
            PUT32(modBlk, dOfs + 76, (uint32_t)codeAdr);
            PUT32(modBlk, dOfs + 80, (uint32_t)refsAdr);
            /* counts */
            PUT32(modBlk, dOfs + 84, (uint32_t)m->nofEntries);
            PUT32(modBlk, dOfs + 88, (uint32_t)m->nofCmds);
            PUT32(modBlk, dOfs + 92, (uint32_t)m->nofPtrs);
            PUT32(modBlk, dOfs + 96, (uint32_t)m->nofTds);
            PUT32(modBlk, dOfs + 100, (uint32_t)m->nofImps);
            /* sizes */
            PUT32(modBlk, dOfs + 104, (uint32_t)m->dataSize);
            PUT32(modBlk, dOfs + 108, (uint32_t)m->conSize);
            PUT32(modBlk, dOfs + 112, (uint32_t)m->codeSize);
            PUT32(modBlk, dOfs + 116, (uint32_t)m->refSize);
            /* CRC = 0 */
            /* export desc */
            PUT32(modBlk, dOfs + 124, (uint32_t)m->exportTree.fp);
            PUT32(modBlk, dOfs + 128, (uint32_t)m->exportTree.adr);
            PUT32(modBlk, dOfs + 132, (uint32_t)m->exportTree.nofExp);
            PUT32(modBlk, dOfs + 136, (m->exportTree.nofExp == 0) ? 0
                  : (uint32_t)m->exportTree.dsc[0].Adr);
            /* publics / privates */
            PUT32(modBlk, dOfs + 156, (uint32_t)m->publics);
            PUT32(modBlk, dOfs + 160, (uint32_t)m->privates);
            PUT32(modBlk, dOfs + 164, (uint32_t)m->nofimp);
            PUT32(modBlk, dOfs + 168, (uint32_t)m->nofstrc);
#undef PUT32

            write_bytes(out, modBlk, (size_t)modBlkSize);
            prevmod = m->modDescAdr;
        }

        /* ---- 2) Data arrays block (NewSys) ---- */
        {
            int32_t dataSizePad = m->dataSize + ((-m->dataSize) & 7);
            int32_t subTotal =
                aos_arr_size(m->nofEntries, 4) +
                aos_arr_size(m->nofCmds, AosCommandRecSize) +
                aos_arr_size(m->nofPtrs, 4) +
                aos_arr_size(m->nofTds, 4) +
                aos_arr_size(m->nofImps, 4) +
                aos_arr_size(dataSizePad + m->conSize, 1) +
                aos_arr_size(m->codeSize, 1) +
                aos_arr_size(m->refSize, 1);
            int32_t blkSize = aos_newsys_block_size(subTotal);
            int32_t blkBase = m->base + aos_newmodule_block_size();
            int32_t sysTag = blkBase + 4; /* NewSys tag points to blkBase+4 */

            /* NewSys header (28 bytes) */
            write_i32_le(out, sysTag);                  /* tag -> self+4 */
            write_i32_le(out, blkSize - 4);             /* size - 4 */
            write_i32_le(out, AosMinPtrOfs - 4);        /* sentinel */
            write_i32_le(out, 0);                       /* nil */
            write_i32_le(out, 0);                       /* nil */
            write_i32_le(out, 0);                       /* flags = {} */
            write_i32_le(out, sysTag);                  /* SysBlk tag */

            int32_t written = 28; /* bytes written so far in this block */

            /* Helper: write sub-array header + data + padding */
#define EMIT_ARR_HDR(elems, esz) do { \
    write_aos_arr(out, sysTag, (elems), (esz)); \
} while(0)

            /* Entry array */
            EMIT_ARR_HDR(m->nofEntries, 4);
            for (int32_t i = 0; i < m->nofEntries; i++) write_u32_le(out, m->entries[i]);
            { int32_t dataN = m->nofEntries * 4;
              int32_t arrSz = aos_arr_size(m->nofEntries, 4);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Command array */
            EMIT_ARR_HDR(m->nofCmds, AosCommandRecSize);
            for (int32_t i = 0; i < m->nofCmds; i++) {
                write_bytes(out, m->cmds[i].name, 32);
                /* proc: if has_param, proc=0; else proc=adr */
                write_i32_le(out, m->cmds[i].has_param ? 0 : m->cmds[i].adr);
                /* parproc: if has_param, parproc=adr; else parproc=0 */
                write_i32_le(out, m->cmds[i].has_param ? m->cmds[i].adr : 0);
            }
            { int32_t dataN = m->nofCmds * AosCommandRecSize;
              int32_t arrSz = aos_arr_size(m->nofCmds, AosCommandRecSize);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Pointer offsets array */
            EMIT_ARR_HDR(m->nofPtrs, 4);
            for (int32_t i = 0; i < m->nofPtrs; i++) write_i32_le(out, m->ptrTab[i]);
            { int32_t dataN = m->nofPtrs * 4;
              int32_t arrSz = aos_arr_size(m->nofPtrs, 4);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Type descriptor pointers array */
            EMIT_ARR_HDR(m->nofTds, 4);
            for (int32_t i = 0; i < m->nofTds; i++)
                write_i32_le(out, m->tdescs[i]->tdAdr);
            { int32_t dataN = m->nofTds * 4;
              int32_t arrSz = aos_arr_size(m->nofTds, 4);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Imports (module pointers) array */
            EMIT_ARR_HDR(m->nofImps, 4);
            for (int32_t i = 0; i < m->nofImps; i++)
                write_i32_le(out, m->imports[i]->modDescAdr);
            { int32_t dataN = m->nofImps * 4;
              int32_t arrSz = aos_arr_size(m->nofImps, 4);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Data + Const array */
            EMIT_ARR_HDR(dataSizePad + m->conSize, 1);
            write_bytes(out, m->data, (size_t)(m->dataSize + m->conSize));
            { int32_t dataN = dataSizePad + m->conSize;
              int32_t gap = dataSizePad - m->dataSize; /* padding between data and const */
              /* data section is dataSize bytes, but we need dataSizePad for alignment */
              /* However, m->data already contains dataSize + conSize bytes contiguously.
               * We wrote dataSize+conSize, need dataSizePad+conSize total. */
              if (gap > 0) write_zeros(out, gap);
              int32_t arrSz = aos_arr_size(dataN, 1);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Code array */
            EMIT_ARR_HDR(m->codeSize, 1);
            write_bytes(out, m->code, (size_t)m->codeSize);
            { int32_t dataN = m->codeSize;
              int32_t arrSz = aos_arr_size(m->codeSize, 1);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }

            /* Refs array */
            EMIT_ARR_HDR(m->refSize, 1);
            if (m->refSize > 0) write_bytes(out, m->refs, (size_t)m->refSize);
            { int32_t dataN = m->refSize;
              int32_t arrSz = aos_arr_size(m->refSize, 1);
              int32_t pad = arrSz - AosArrHdrSize - 4 - dataN;
              if (pad > 0) write_zeros(out, pad);
              written += arrSz; }
...
#undef EMIT_ARR_HDR
            ...
            /* Pad remaining NewSys block to blkSize */
            int32_t blkPad = blkSize - 28 - subTotal;
            if (blkPad > 0) write_zeros(out, blkPad);
        }

        /* ---- 3) Export padding + export dump ---- */
        {
            int32_t next = m->base + aos_newmodule_block_size() +
                           aos_newsys_block_size(
                               aos_arr_size(m->nofEntries, 4) +
                               aos_arr_size(m->nofCmds, AosCommandRecSize) +
                               aos_arr_size(m->nofPtrs, 4) +
                               aos_arr_size(m->nofTds, 4) +
                               aos_arr_size(m->nofImps, 4) +
                               aos_arr_size(m->dataSize + ((-m->dataSize) & 7) + m->conSize, 1) +
                               aos_arr_size(m->codeSize, 1) +
                               aos_arr_size(m->refSize, 1));

            /* export padding + sentinel tag slot */
            write_bytes(out, padding, (size_t)(m->expPadding + 4));
            next += m->expPadding + 4;
            assert(((uint32_t)next % 32) == 0);
            /* AOS: export tag = 0 (no expDescSF) */
            dump_export(out, &m->exportTree, &next, 0 | 2);
...
static void load_module_body(FILE *f, Module *m, int32_t *base) {
    ...
    /* ---- AOS image layout computation ---- */
    m->base = *base;
    insert_module_sorted(m);

    /* 1) Module descriptor block (NewModule – protected object) */
    int32_t modBlkSize = aos_newmodule_block_size(); /* 224 */
    m->modDescAdr = m->base + 4 + AosProtOfs;
    m->imageSize = modBlkSize;

    /* 2) Data arrays block (AllocateModule – NewSys wrapping sub-arrays) */
    int32_t dataSizePad = m->dataSize + ((-m->dataSize) & 7); /* round up to 8 */
    int32_t subTotal =
        aos_arr_size(m->nofEntries, 4) +
        aos_arr_size(m->nofCmds, AosCommandRecSize) +
        aos_arr_size(m->nofPtrs, 4) +
        ...
        aos_arr_size(m->codeSize, 1) +
        aos_arr_size(m->refSize, 1);
    int32_t dataBlkSize = aos_newsys_block_size(subTotal);

    /* Compute sub-array offsets inside the data block */
    int32_t dataBlkBase = m->base + modBlkSize; /* start of NewSys block */
    int32_t arrOfs = dataBlkBase + 28; /* first sub-array (after NewSys header) */
    ...
    arrOfs += aos_arr_size(m->nofImps, 4);
    /* data+const */
    int32_t dataArrAdr = arrOfs;
    arrOfs += aos_arr_size(dataSizePad + m->conSize, 1);
    /* code */
    int32_t codeArrAdr = arrOfs;
    arrOfs += aos_arr_size(m->codeSize, 1);
    /* refs */
    int32_t refsArrAdr = arrOfs;
    (void)refsArrAdr; /* used in dump_modules */

    m->sb = dataArrAdr + AosArrHdrSize + dataSizePad;
    m->codeBase = codeArrAdr + AosArrHdrSize;
    m->refBase = refsArrAdr + AosArrHdrSize;

    m->imageSize += dataBlkSize;
...
static void usage(FILE *out) {
    fprintf(out,
            "usage: aosbootlinker [options] <Module1> <Module2> ...\n"
            "\n"
            "Options:\n"
...
int main(int argc, char **argv) {
    /* Endianness check: DumpModuleDesc is written as raw bytes, must be LE */
    {
        ...
        if (*(uint8_t *)&endian_test != 1) {
            fprintf(stderr, "aosbootlinker: this tool requires a little-endian host\n");
            return 1;
        }
    }
